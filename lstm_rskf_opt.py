...
# =============================================================================
# 2. Definir arquitectura de la LSTM con hiperparámetros
# =============================================================================
def build_model(hp):
    model = Sequential()
    model.add(LSTM(units=hp.Int("units_lstm", 32, 128, step=32), return_sequences=False, input_shape=(1, 32)))
    
    model.add(Dropout(hp.Float("dropout_1", 0.2, 0.5, step=0.1)))
    
    model.add(Dense(units=hp.Int("units_1", 16, 64, step=16), activation=hp.Choice("activation_1", ["relu", "tanh"])))
       
    model.add(Dense(y_encoded.shape[1], activation="softmax"))
    
    model.compile(optimizer=Adam(learning_rate=hp.Choice("learning_rate", [0.01, 0.001, 0.0001])), loss="categorical_crossentropy", metrics=["accuracy"])
    return model
...
for batch_size in batch_sizes:
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X, y)):
        ...
        tuner = kt.RandomSearch(
            ...
            project_name=f"lstm_rskf{fold}_bs{batch_size}"
            )   
        ...
        best_model.save(os.path.join(ruta,(f"lstm_bm_kf{fold}_bs{batch_size}.keras")))
        ...
        with open(os.path.join(ruta, f"lstm_bhf{fold}_bs{batch_size}_hy.json"), "w") as f:
            json.dump(histories_series, f, indent=4)
        
        hps_per_batch[batch_size] = hps_per_fold
        with open(os.path.join(ruta, f"lstm_bhf{fold}_bs{batch_size}.json"), "w") as f:
    ...
    print(f"🔹 batch_size = {batch_size} → Exactitud: {mean_acc:.4f} ± {std_acc:.4f}, Pérdida: {mean_loss:.4f} ± {std_loss:.4f}")

# 🔹 Guardar resultados en JSON para análisis posterior
with open(os.path.join(ruta,"lstm_batch_size_results.json"), "w") as f:
    json.dump(results, f, indent=4)

# 🔹 Imprimir resumen
print("\n🔹 Comparación de batch_size:")
for r in results:
    print(f"batch_size = {r['batch_size']}: Exactitud = {r['mean_accuracy']:.4f} ± {r['std_accuracy']:.4f}")
...
final_model.save(os.path.join(ruta, "lstm_bm_gkv.keras"))
with open(os.path.join(ruta, "lstm_bm_hps.json"), "w") as f:
...
# =============================================================================
print(f"Media de Exactitud en validación cruzada: {np.mean(fold_accuracies):.4f} ± {np.std(fold_accuracies):.4f}")
print(f"Media de pérdida en validación cruzada: {np.mean(fold_losses):.4f} ± {np.std(fold_losses):.4f}")
print(f"Media de Exactitud en entrenamiento: {np.mean(train_accuracies):.4f} ± {np.std(train_accuracies):.4f}")
print(f"Media de pérdida en entrenamiento: {np.mean(train_losses):.4f} ± {np.std(train_losses):.4f}")
