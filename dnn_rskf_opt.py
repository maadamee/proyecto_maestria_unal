...
for batch_size in batch_sizes:
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X, y)):
        ...
        tuner = kt.RandomSearch(
            ...
            project_name=f"dnn_rskf{fold}_bs{batch_size}"
            )   
        ...
        best_model.save(os.path.join(ruta,(f"dnn_bm_kf{fold}_bs{batch_size}.keras")))
        ...
        with open(os.path.join(ruta, f"dnn_bhf{fold}_bs{batch_size}.json"), "w") as f:
...
final_model.save(os.path.join(ruta, "dnn_bm_gkv.keras"))
with open(os.path.join(ruta, "dnn_bm_hps.json"), "w") as f:
